...
class NonAdministratorTest(LiveServerTestCase):
    def setUp(self):
        User.objects.create_user(username=other_username, password=other_password, first_name=other_first_name, last_name=other_last_name, email=other_email)
        self.browser = webdriver.Firefox()
        self.browser.implicitly_wait(3)
    ...
    def test_features(self):
        ...
        usernameBox.send_keys(other_username)
        passwordBox.send_keys(other_password)
        submitButton.click()
        ...
        self.assertEqual('Home', self.browser.title)
        self.check_current_date_and_fullname_display()
        fullname = other_first_name + " " + other_last_name
        self.assertIn(fullname, self.browser.find_element_by_tag_name('body').text )
        #empty todo list check
        self.assertIn("To do list is empty", self.browser.find_element_by_tag_name('body').text )
